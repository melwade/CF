...
?><script>dhtmlx.alert("There was an error processing your request.");</script><? 
  ...
  include('CF_header.php'); ?>
      ...
      <h1 class="Heading-1" style="margin-bottom:25px;">Expired Members</h1>
	  <?
		include ('connect.php');
    ...
    /*	$result = mysql_query("SELECT id, fname, lname, payment_status, expiration_date,
	    ...
	    FROM tblUserData
		WHERE
		payment_status='Expired' AND
	    1=1 ORDER BY expiration_date DESC");
		*/
		
		$result = mysql_query("SELECT id, id, fname, lname, payment_status, expiration_date,
			DATEDIFF(DATE(expiration_date), CURDATE()) as 'DaysTillRenewal'
		FROM tblUserData
		WHERE
			DATEDIFF(DATE(expiration_date), CURDATE()) < -60 ORDER BY DaysTillRenewal DESC");
	    
		echo "<table bgcolor='#a20000' border='0' width='650'><tr bgcolor='#a20000'><th class='jtable'><font color='#fff'>Name</font></th><th class='jtable'><font color='#fff'>Payment Status</font></th><th class='jtable'><font color='#fff'>Expiration Date</font></th><th class='jtable'><font color='#fff'>Exp Days</font></th><th class='jtable'><font color='#fff'>Action</font></th></tr>";
		while ($row = mysql_fetch_array($result)) {
			...
			echo "<td class='jtable' align='center' width='100'>$row[DaysTillRenewal]</td>";
			echo "<td align='center'> <a href='edit_member.php?id=$row[id]'><img alt='Edit Item' width='18px' title='Edit Item' src='images/edit-icon.png'></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a onclick='return confirmSubmit()' href='delete_member_expired_users.php?id=$row[id]'><img alt='Delete Item' title='Delete Item' width='18px' src='images/delete-icon.png'></a></td>";
			echo "</tr>"; }

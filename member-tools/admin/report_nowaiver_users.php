...
?><script>dhtmlx.alert("There was an error processing your request.");</script><? 
  ...
  include('CF_header.php'); ?>
      ...
      <h1 class="Heading-1" style="margin-bottom:25px;">No Waiver Members</h1>
	  <?
		include ('connect.php');
    ...
    /*	$result = mysql_query("SELECT id, fname, lname, payment_status, expiration_date,
      ...
      */
		...
		while ($row = mysql_fetch_array($result)) {
			...
			//CHECK TO MAKE SURE USER DOESN'T HAVE A NO WAIVER STATUS AND A DATE THAT HASN'T EXPIRED
			if(date("Y-m-d") < $row['expiration_date']){
			echo "<tr bgcolor=$bgcolor ><td align='center' class='jtablered' width='200'>$row[fname]". " ". "$row[lname]</td>";
			echo "<td class='jtablered' align='center' width='200'>$row[payment_status]</td>";
			echo "<td class='jtablered' align='center' width='100'>$row[expiration_date]</td>";
			echo "<td align='center'> <a href='edit_member.php?id=$row[id]'><img alt='Edit Item' width='18px' title='Edit Item' src='images/edit-icon.png'></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a onclick='return confirmSubmit()' href='delete_member_nowaiver_users.php?id=$row[id]'><img alt='Delete Item' title='Delete Item' width='18px' src='images/delete-icon.png'></a></td>";
			echo "</tr>";	
			}
			else {

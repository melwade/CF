...
th.jtable {padding: 5px; font-size:14px; text-align:center !important;} /* or you can target individual cells */
  ...
  <title>Ride Leader Family Memberships</title>
  <!-- CSS -->
...
?><script>dhtmlx.alert("There was an error processing your request.");</script><? 
  ...
  include('CF_header.php'); ?>
      ...
      <h1 class="Heading-1" style="margin-bottom:25px;">Ride Leader Family Memberships</h1>
      <p>Spouses of Ride Leaders are eligible for this status.</p>
	  <?
		include ('connect.php');
    ...
    /*	$result = mysql_query("SELECT id, fname, lname, payment_status, expiration_date,
      ...
      */
	    ...
	    FROM tblUserData
		WHERE
		user_level='Ride Leader Family' AND
	    1=1 ORDER BY lname, fname ASC");
		...
		while ($row = mysql_fetch_array($result)) {
			...
			//CHECK TO MAKE SURE USER DOESN'T HAVE AN UNPAID STATUS AND A DATE THAT HASN'T EXPIRED
			...
			echo "<td align='center'> <a href='edit_member.php?id=$row[id]'><img alt='Edit Item' width='18px' title='Edit Item' src='images/edit-icon.png'></a></td>";
			echo "</tr>";	
			}
			else {
			echo "<tr bgcolor=$bgcolor ><td align='center' class='jtable' width='200'>$row[fname]". " ". "$row[lname]</td>";
			echo "<td class='jtable' align='center' width='200'>$row[payment_status]</td>";
			echo "<td class='jtable' align='center' width='100'>$row[expiration_date]</td>";
			echo "<td align='center'> <a href='edit_member.php?id=$row[id]'><img alt='Edit Item' width='18px' title='Edit Item' src='images/edit-icon.png'></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a onclick='return confirmSubmit()' href='delete_member_unpaid_users.php?id=$row[id]'><img alt='Delete Item' title='Delete Item' width='18px' src='images/delete-icon.png'></a></td>";
			echo "</tr>"; }

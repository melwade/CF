...
$sql="SELECT * FROM tblUserData WHERE email_add = '$myusername'";
	...
	$insert_query="insert into tblResets (account_id, email, expire_date, reset_code) values ('$user_username', '$user_email', '$expire_date', '$reset_code')";
	$result = mysql_query($insert_query) or die('Error inserting record: ' . mysql_error());
	
	//Send email with reset info
	send_password_reset($user_email, $reset_code, $ip_add);
